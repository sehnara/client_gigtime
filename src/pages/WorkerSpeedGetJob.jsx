...
const mockData = [
    138300, 138750, 138900, 140100, 141150, 141850, 143200, 144250, 144950, 146300, 147350, 148050, 149400, 150450, 151150, 151600, 152500,
    153550, 154250, 154300, 154700, 155600, 156650, 157350, 157400, 157800, 158100, 158900, 159600, 159700, 160100, 160500, 160600, 161100,
    161800, 162300, 162400, 162800, 163200, 163400, 164100, 164600, 165100, 165200, 165600, 166000, 166200, 166700, 167200, 167700, 168200,
    168300, 168700, 169100, 169300, 169550, 171350, 172050, 172550, 173050, 173550, 174050, 174200, 174550,
];
...
const setTimes = (start, end) => {
    const arr = [];
    if (start !== '' && start !== '') {
        for (let i = Number(start.split(':')[0]); i <= Number(end.split(':')[0]); i++) {
            if (i < 10) {
                arr.push(`0${i}:00`);
            } else {
                arr.push(`${i}:00`);
...
const WorkerSpeedGetJob = () => {
    ...
    return (
        <div>
            {isOpen && (
                <div onClick={() => setIsOpen(!isOpen)} className="flex bg-black/[.8] absolute w-full h-full ">
                    <Pachinco result={result} moneys={result.p_history} speed={80} visits={result.visit} />
                </div>
            )}
            <Header title="바로알바" />
            <div className="mx-8">
                ...
                />
                <Button title={'바로알바 견적내기'} onClickEvent={() => setIsPopUp(!isPopUp)} />
            </div>
        </div>
    );
};
